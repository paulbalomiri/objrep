...
public class FactoryTest {
	...
	private boolean testAddBand() {
		...
		try {
			...
			writeError("Could not Instantiate, insert or reread Band from DB:" + e.getDescription());
			ret = false;
		}
		if (ret) {
	...
	private boolean testFactoryMethodAvailability() {
		boolean ret = true;
		for (Class<?> c : DB.getTypeMap().keySet()) {
			...
			if (req == null) {
				...
				Class<?> implClass =  DB.getTypeMap().get(c);
